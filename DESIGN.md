...
**Status:** stages one to five of `§10` on Linux: the core, both validators, the session, and a window whose main pane is the document rendered from the tree and edited in place, with the page image as a side panel. Two keyboard walkthroughs done. Nothing is packaged; stage six is next.
