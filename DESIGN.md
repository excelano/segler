...
Cloned from slipcase-desktop, one directory per platform under `packaging/`, with that repository's `store-listing.md` as the template. What was measured there is taken as measured here: the Visual C++ runtime linked in with `+crt-static` and the import table checked before packaging; the Linux `Depends` checked against `/proc/PID/maps` on both Wayland and X11; the Mac bundle with the App Sandbox; `CFBundleVersion` from the first-parent commit count. A Store build needs slipcase-desktop's pinned `[patch.crates-io]` on `excelano/winit` until a winit release ships the upstream fix behind its `private-apple-apis` feature, because `winit` 0.30 declares a private CoreGraphics symbol whether or not it is called and the Mac App Store refuses it under Guideline 2.5.1. `build-app.sh` refuses any binary importing a symbol no public framework header declares.
