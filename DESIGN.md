...
**Status:** stages one to five of `§10` done, and the Linux half of stage six: the core, both validators, the session, a window whose main pane is the document rendered from the tree and edited in place, and a Debian package that passes lintian with its dependencies measured against a running window. Two keyboard walkthroughs done. The Windows and Mac halves of stage six are next, each on its lane.
