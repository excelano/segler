...
#
# All three crates are scanned though only `segler-desktop` draws anything.
# `segler-core` is the model and the CLI prints for a terminal, and a message
# turning up in either is either a mistake or the day that changed; scanning
# them means the catalogue says which.
#
...
# in a comment. The warnings cost nothing, but they are also where a genuine
# miss would hide, so the way a dropped string is found is not by reading them
# — it is `pseudo.sh` beside this, where anything still in English stands out
# on sight.
set -eu

# Three directories up: this script sits in the crate that owns the
# catalogues, and the sources it scans are the whole workspace's.
...
# 2026-09-09 by a failed flyleaf release, and this repository publishes its
# three crates the same way.
pot="crates/segler-desktop/po/$domain.pot"
...
sources=$(find crates -name '*.rs' | sort)
...
xgettext \
    ...
    --sort-by-file \
    --package-name="$domain" \
    --msgid-bugs-address=https://github.com/excelano/segler/issues \
    --output="$pot" \
    $sources
...
for catalogue in crates/segler-desktop/po/*.po; do
