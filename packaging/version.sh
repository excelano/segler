...
#
# The workspace `Cargo.toml` holds the number and nothing else should. Three
...
# The first `version =` at the start of a line, which in a workspace manifest
# is `[workspace.package]`'s; the crates inherit it and carry none of their own.
